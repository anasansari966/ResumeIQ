...
function SectionCard({ title, children, delay = 0 }) {
  return (
    <section
      className="resume-preview-reveal rounded-3xl border border-black/5 bg-white/80 p-4 shadow-[0_14px_32px_rgba(15,23,42,0.05)]"
...
function ModernPreview({ data, template, variant }) {
  ...
  return (
    ...
    >
      <div
        className="resume-preview-paper animate-float-soft relative overflow-hidden rounded-[24px] border border-black/5 shadow-[0_30px_80px_rgba(15,23,42,0.16)]"
                    ...
                    <article key={`${item.company || "exp"}-${idx}`} className="flex gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-[12px] text-[color:var(--preview-muted)]">
                          {[item.company, item.start_date || item.startDate, item.end_date || item.endDate].filter(Boolean).join(" | ") ||
                            "Company | Recent"}
                        </p>
                        {highlights.length ? (
                          ...
                          <p className="mt-2 text-[12px] leading-5 text-[color:var(--preview-muted)]">
                            - Measurable role impact will appear here in the animated preview.
                          </p>
                        )}
                      </div>
                    </article>
              ...
              <div className="space-y-3">
                {data.education.length ? (
                  data.education.map((item, idx) => (
                    <article
                      key={`${item.institution || item.school || "edu"}-${idx}`}
                      className="rounded-2xl border border-black/5 bg-black/[0.015] px-3 py-3"
                    >
                      <p className="text-sm font-semibold text-[var(--preview-ink)]">{item.degree || item.program || "Degree"}</p>
                      <p className="mt-0.5 text-[12px] text-[color:var(--preview-muted)]">
                        {[item.institution || item.school, item.year || item.graduation_year].filter(Boolean).join(" | ") ||
                          "Institution | Year"}
                      </p>
                    </article>
                  ))
...
function SidebarPreview({ data, template }) {
  ...
  return (
    ...
    >
      <div
        className="resume-preview-paper animate-float-soft grid min-h-full overflow-hidden rounded-[24px] border border-black/5 shadow-[0_30px_80px_rgba(15,23,42,0.18)] lg:grid-cols-[220px,1fr]"
        style={{ background: theme.paper }}
      >
        <div className="resume-preview-sheen" />
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[0.9fr,1.1fr]">
            <SectionCard title="Education" delay={160}>
              <div className="space-y-3">
                {data.education.length ? (
                  data.education.map((item, idx) => (
                    <article key={`${item.institution || item.school || "edu"}-${idx}`}>
                      <p className="text-[12px] font-semibold text-[var(--preview-accent)]">
                        {item.year || item.graduation_year || "Year"}
                      </p>
                      <p className="mt-1 text-sm font-semibold text-[var(--preview-ink)]">{item.degree || item.program || "Degree"}</p>
                      <p className="mt-0.5 text-[12px] text-[color:var(--preview-muted)]">{item.institution || item.school || "Institution"}</p>
                    </article>
                  ))
                ) : (
                  <p className="text-sm text-[color:var(--preview-muted)]">Add education to populate this preview.</p>
                  ...
                  data.experience.map((item, idx) => {
                    ...
                    return (
                      <article key={`${item.company || "exp"}-${idx}`}>
                        <p className="text-[12px] font-semibold text-[var(--preview-accent)]">
                          {[item.start_date || item.startDate, item.end_date || item.endDate].filter(Boolean).join(" - ") || "Recent"}
                        </p>
                        ...
                        {highlights.length ? (
                          ...
                          <p className="mt-2 text-[12px] leading-5 text-[color:var(--preview-muted)]">
                            - Role impact details will appear here in the preview.
                          </p>
                        )}
                      </article>
                    );
